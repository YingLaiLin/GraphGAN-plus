...
class GraphGan(object):
    ...
    def sample_for_gan(self, root, tree, sample_num, all_score, sample_for_dis):
        ...
        Args:
            ...
            sample_for_dis: bool, indicates it is sampling for generator or
            discriminator
        Returns:
        ...
        while len(sample) < sample_num:
            ...
            while True:
                ...
                if node_neighbor == []:  # the tree only has the root
                    return sample
                if sample_for_dis == True:  # only sample the negative
                    # examples for discriminator, thus should exclude the
                    # root node tobe sampled
                ...
                external_prob = np.array([self.probs[node_select, neighbor] for neighbor in node_neighbor])
                external_prob = utils.softmax_norm(external_prob)
                prob = config.fraction_of_softmax * prob + (
                            1 - config.fraction_of_softmax) * external_prob
                prob /= prob.sum()
    ...
    def mul_construct_trees(self, nodes):
        ...
        if config.use_mul:
            ...
            for i in range(cores):
                if i != cores - 1:
                    new_nodes.append(
                        nodes[i * node_per_core:(i + 1) * node_per_core])
                else:
    ...
    def construct_tree(self, nodes):
        ...
        "Node": [father, children], if node is the root, then the father is
        itself.
        Args:
            nodes:
        Returns:
            trees: dict, <key, value>:<node_id, {dict(store the neighbor
            nodes)}>
        """
        trees = {}
        for root in tqdm.tqdm(nodes):
            if root not in self.linked_nodes:
                continue
            trees[root] = {}
            ...
            queue = collections.deque(
                tmp)  # the nodes in this queue all are items
            for x in tmp:
                trees[root][x] = [root]
            used_nodes = set(tmp)
            used_nodes.add(root)
    ...
    def build_gan(self):
        ...
        self.latest_checkpoint = tf.train.latest_checkpoint(
            config.model_log_dir)
        # add the saver
        self.saver = tf.train.Saver()

    def generate_for_d(self):
        """Generate the pos and neg samples for the Discriminator, and record
        ...
        for u in self.root_nodes:
            if np.random.rand() < config.update_ratio:  #
                ...
                neg = self.sample_for_gan(u, self.trees[u], len(pos), all_score,
                                          sample_for_dis=True)
                if len(neg) < len(pos):
                    continue
    ...
    def train_gan(self):
        ...
        ckpt = tf.train.get_checkpoint_state(config.model_log_dir)
        ...
        for epoch in tqdm.tqdm(range(config.max_epochs)):
            ...
            if epoch % config.save_steps == 0 and epoch > 0:
                model_log_filename = ".." + config.model_log_dir
                if not os.path.isdir(model_log_filename):
                    os.mkdir(model_log_filename)
                self.saver.save(self.sess, model_log_filename + "/model.ckpt")

            for d_epoch in tqdm.tqdm(range(config.max_epochs_dis)):
                if d_epoch % config.gen_for_d_iters == 0:  # every
                    # gen_for_d_iters round, we generate new data
                    self.generate_for_d()
                train_size = len(self.samples_q)
                #  traverse the whole training dataset sequentially,
                ...
                while True:
                    ...
                    if index + config.batch_size_dis <= train_size + 1:
                        input_q_node, input_rel_node, input_label = self.get_batch_data(
                            index, config.batch_size_dis)
                    else:
                        input_q_node, input_rel_node, input_label = self.get_batch_data(
                            index, train_size - index)
                    index += config.batch_size_dis
                    _, loss = self.sess.run([self.discriminator.d_updates,
                                             self.discriminator.pre_loss], {
                                                self.discriminator.q_node: np.array(
                                                    input_q_node), \
                                                self.discriminator.rel_node: np.array(
                                                    input_rel_node),
                                                self.discriminator.label: np.array(
                                                    input_label)
                                                })

            for g_epoch in tqdm.tqdm(range(config.max_epochs_gen)):
                cnt = 0
                root_nodes = []  # just for record how many trees that have
                # been traversed
                rel_nodes = []  # the sample nodes of the root node
                root_nodes_gen = []  # root node feeds into the network,
                # same length as rel_node
                trace = []  # the trace when sampling the nodes, from the
                # root to leaf  bach to leaf's father. e.g.: 0 - 1 - 2 -1
                all_score = self.sess.run(
                    self.generator.all_score)  # compute the score for
                # computing the probability when sampling nodes
                for root_node in tqdm.tqdm(self.root_nodes,
                                           mininterval=3):  # random update
                    # trees
                    if np.random.rand() < config.update_ratio:
                        # sample the nodes according to our method.
                        # feed the reward from the discriminator and the
                        # sampled nodes to the generator.
                        if cnt % config.gen_update_iter == 0 and cnt > 0:
                            # generate update pairs along the path, [q_node,
                            # rel_node]
                            pairs = list(map(self.generate_window_pairs,
                                             trace))  # [[], []] each list
                            # contains the pairs along the same path
                            q_node_gen = []
                            rel_node_gen = []
                            ...
                            reward_gen, node_embed = self.sess.run(
                                [self.discriminator.score,
                                 self.discriminator.node_embed], {
                                    self.discriminator.q_node: np.array(
                                        q_node_gen),
                                    self.discriminator.rel_node: np.array(
                                        rel_node_gen)
                            ...
                            feed_dict = {
                                self.generator.q_node: np.array(q_node_gen),
                                self.generator.rel_node: np.array(rel_node_gen),
                                self.generator.reward: reward_gen
                                }
                            _, loss, prob = self.sess.run(
                                [self.generator.gan_updates,
                                 self.generator.gan_loss,
                                 self.generator.i_prob], feed_dict=feed_dict)
                            all_score = self.sess.run(self.generator.all_score)
                            root_nodes = []
                            rel_nodes = []
                            root_nodes_gen = []
                            trace = []
                            cnt = 0
                        if root_node not in self.trees:
                            continue
                        sample = self.sample_for_gan(root_node,
            ...
            print("Evaluation")
            self.write_emb_to_txt()  # self.eval_test()  # evaluation

    def generate_window_pairs(self, sample_path):
        """
        given a sample path list from root to a sampled node, generate all
        the pairs corresponding to the windows size
        e.g.: [1, 0, 2, 4, 2], window_size = 2 -> [1, 0], [1, 2], [0, 1], [0,
        2], [0, 4], [2, 1], [2, 0], [2, 4], [4, 0], [4, 2]
        :param sample_path:
        :return:
        """
        ...
        for i in range(len(sample_path)):
            ...
            for j in range(max(i - config.window_size, 0),
                           min(i + config.window_size + 1, len(sample_path))):
                if i == j:
                    continue
                node = sample_path[j]
...
if __name__ == "__main__":
    g_g = GraphGan()
    g_g.train_gan()
    send_mail.send_mail("GraphGAN", "program finished")
